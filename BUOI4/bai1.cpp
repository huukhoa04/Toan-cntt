...
void Cholesky_Decomposition(float matrix[][MAX], float output1[][MAX], float output2[][MAX], int n)
{
    ...
    // Decomposing a matrix into output1 Triangular
    for (int i = 0; i < n; i++) {
        for (int j = 0; j <= i; j++) {
            float sum = 0;
        ...
        transpose(output1, n, n, output2);
    // Displaying output1 Triangular and its Transpose
    cout << setw(15) << " L " 
         << setw(31) << "LT" << endl;
    for (int i = 0; i < n; i++) {
...
int main()
{
    ...
    cout << "LLT(Cholesky) DECOMPOSITION: \n";
    // float Transpose[MAX][MAX];
    Cholesky_Decomposition(matrix, L1, L2, n);
    Reset(L1, n);
    Reset(L2, n);
    Reset(L3, n);
    // print(matrix, n);
    // transpose(matrix, n, n, Transpose);
    // print(Transpose, n);
    // print(L1, n);
    // print(L2, n);
    // cout << (check(matrix, L1, L2, n)? "true" : "false") << "\n";
